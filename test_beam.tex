...
\minitoc
  ...
  \label{sec:X0}
   ...
   \subsection{Software analysis chain}
    ...
    It is based on the MARLIN framework, which is a part of the ILCSOFT.
    The software is used to convert the dart into the LCIO format, but also to perform clustering search, alignment and final analysis.
    Each step of the analysis is driven by a dedicated processor.

    From the raw data files, a first processor is used to convert the data into the LCIO format.y
   ...
   % Firstly, the raw data files are converted into the LCIO data format to provide a consistency data format during the analysis step.
   % This file contains the pixel number which fired in a given event, along with the sensor number.
   % Then, a processor removing the noisy pixel is applied, before using a cluster algorithm to form sets of cluster from the individual activated pixels.
   % A centre-of-gravity algorithm is then applied to 
