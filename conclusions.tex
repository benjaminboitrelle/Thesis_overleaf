...
Over the last few years, the scientific community has came to the agreement that an accelerator beyond the \gls{LHC} is needed.
This next generation accelerator is required for high-precision measurements, specifically for characterising the electroweak symmetry breaking, as well as looking for physics beyond the \gls{SM}.
...
A decision from the Japanese government is eagerly awaited to start the project.
The work performed during this thesis was specifically focused on the feasibility of double-sided pixelated silicon ladders suited for a vertex detector at the \gls{ILC} that feature a low material budget (below $0.35~\%~\rm{X_{0}}$) and a spatial resolution below $ 3~\rm{\mu m}$ for the \gls{ILD}.

This work begins with a physics analysis of simulated data corresponding to collisions in the \gls{ILC} at a centre-of-mass energy $\sqrt{s} = 350~\rm{GeV}$.
...
A corner stone of the analysis of this channel is the ability to drastically reduce the contribution of events leading to the same detector response or a similar event signature. 
...
Though sequential cuts are able to reduce the background by three orders of magnitude, they also remove half of the signal. 
...
After this first important selection step, the actual characterization of the Higgs boson can start. 
