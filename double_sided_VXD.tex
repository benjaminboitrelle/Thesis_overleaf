...
\label{chap:vxd}
  ...
  % At the moment the mechanical structure that holds the two layers is 2 mm thick, but some geometry optimisation depending on the physics to study might consider having closer layers on the first ladder, while the last ladder could have a thicker mechanical structure.
   ...
   \subsubsection{DEPFET}
    ...
    The sensor is completely depleted of free charged carries thanks to a voltage applied along the thickness.
  ...
  In 2016, the DESY has provided the opportunity to test the ladder in real conditions thanks to the test beam facility and the possibility to use the \gls{DAQ} software developed at DESY: EUDAQ.
    ...
    The advantages of this technique are, firstly, the direct connection of metal traces to the pads that avoid wire-bonding and can reduce, in the same time, the width of the module.
  ...
  %They are called monolithic because the sensitive volume and the microelectronic circuitry form one physical block.
    ...
    The silicon used has a good quality, thus the charge carriers have less change to recombine.
    ...
    As no external voltage is applied to the sensor to increase the depleted region, the charge carriers created by crossing particles are thermally diffused inside the epitaxial layer to the diode.
    ...
    Hence, the signal created is only few millivolts and low noise electronics have to be used for processing the signal.
   ...
   % Although the compensation is fast, it generates a dead time for detection between two readings.
    ...
    The advantage of such method is that the discrimination can be adjusted offline for each pixel, thus compensating the non uniform response.
    ...
    An analog output is then too slow for an \gls{ILC} purpose. 
    ...
    Instead of having one bus line for the whole matrix, each columns has in own bus and a data sparsification logic is integrated on the periphery of the sensor.
    One row is read in only 100 ns, independently of the number of pixels contained.
    When one line is read, the precedent line is processed by the electronics on the bottom each columns.
    Only the pixels having a signal above a certain thresholds are read.
    A zero suppression logic, called \gls{SUZE}, is in charge to determine the number of fired pixels in a raw and to send the address of the first pixel hit, as shown on figure~\ref{fig:SUZE}.
    ...
    \begin{figure}[!h]
      \missingfigure{SUZE}
      \caption{Zero suppression principle.}
      \label{fig:SUZE}
    \end{figure}
