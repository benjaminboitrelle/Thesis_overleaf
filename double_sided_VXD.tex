...
\label{chap:vxd}
  ...
  \section{The ILD vertex detector specifications}
    ...
    Depending on the option chosen (see section~\ref{subsec:design}), the \gls{VXD} has to provide five or six points of measurement with a very high precise spatial resolution.
   ...
   %The resolution on the impact parameter $\sigma_{IP}$, defined as the distance of closest approach of the particle to the colliding beam can be parametrised by the formula~\ref{eq:resIP}.\todo{REPHRASE}
    ...
    Where $\theta$ is the track polar angle, $a$ and $b$ are explained in the following.\todo{Define geometry}
   ...
   Different options for \gls{CMOS} pixel sensors are studied, such as the 3D integrated \gls{CMOS}, but due to the context of this thesis, the work will focus on the \gls{CMOS} sensors based on the \gls{MIMOSA} architecture developed by the \gls{IPHC} of Strasbourg. 
  ...
  In 2016, DESY has provided the opportunity to test the ladder in real conditions thanks to the test beam facility and the possibility to use the \gls{DAQ} software developed at DESY: EUDAQ \cite{EUDAQ}.
    ...
    Figure~\ref{gih:YongModulus} represents the Young Modulus as a function of the radiation length for different materials.
    ...
    Then in 2010, a second prototype featuring the wanted sensitive surface, called version-1 (V-1), was developed.
  ...
  \begin{figure}[!h]
    ...
    The PICSEL group has then developed digital output sensors for the pixel vertex detector at the START experiment at Brookhaven National Laboratory \cite{1748-0221-7-01-C01102}\cite{1748-0221-10-03-C03026}.
