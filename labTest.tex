...
\section{PLUME assembly procedures}
  ...
  As explain in section~\ref{subsec:modAssembly}, the sensors positioning was performed firstly manually and later was switched to an automatic procedure.
...
\section{Electrical validation}
  ...
  A module or a PLUME ladder is connected to the outside world by plugging a jumper cable on a \gls{ZIF} connector at one edge of the module.
  ...
  The second RJ45 connector is providing signal coming from the \gls{DAQ}:
  \begin{itemize}
  ...
  Then, the $\rm{V}_{\rm{CC}}$, $\rm{V}_{\rm{DD_D}}$ and $\rm{V}_{\rm{DD_A}}$ should be at $3.3~\rm{V}$, but only the two last voltages can be adjusted by using two potentiometers on the power supply board.
  The $\rm{V}_{\rm{clp}}$ is set to $2.1~\rm{V}$ and should not be outside the range $\left[2, 2.2\right]~\rm{V}$.
  The \gls{JTAG} communication is verified on an oscilloscope.
