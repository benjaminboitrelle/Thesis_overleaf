...
\section{Electrical validation}
 ...
 % Two RJ45 are in charge to provide the JTAG registers, as well as the start, the reset signal and in a case of a complete ladder an external clock.
  ...
  %\begin{itemize}
  %  \item Generate the 80 MHz for the 6 Mi 26 by an oscillator on board or by an external input.
 ...
 % After the connections were controlled and before to connect the module to the bench, the different voltages have to be set.
...
\section{Noise measurements}
  ...
  The noise performance of the sensor is determined thanks to a threshold scan around the "middle-point" found before.
  ...
  % Noise performance of full in-pixel circuitry + sensing system and discriminator characterised by using the S-curve method: normalised response as a function of the threshold voltage.
  %To measure the noise performance of the pixels and the discriminators, a threshold scan is performed around the "middle-point" found before.
  %For each step, the threshold voltage is adjusted and the data acquisition software records the normalised response of the system (pixels + discriminators response).
